...
streams.  For instance, L7mp supports many protocols that Envoy does
...
protocol clusters, with automatic and transparent protocol
transformation (e.g., you can route an UDP listener or a raw IP socket
to a WebSocket cluster and everything should work as expected), native
support for datagram- and byte-streams, stream
...
additions, like support for listing, killing, or dynamically
re-routing(!)  individual transport session.
