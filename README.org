...
/L7mp is under construction, with many advertised features untested,
not working as promised, or completely missing./
...
- "meta", because new protocols can dynamically added on top of
  primitive protocols types;
...
L7mp is modeled after [[https://github.com/envoyproxy/envoy][Envoy]] in that it uses similar abstractions
(Listeners, Clusters, etc.), but it adds some new functionality to
manage long-lived media streams, including off-the-shelf support
protocols that Envoy does not support natively, or does not support at
all (e.g., UDP, raw IP, L2), and the capacity to build new ones
(VXLAN, RTP, DNS, or ICMP) and to transparently route sessions across
such protocol clusters with transparent protocol transformation (e.g.,
to route an UDP listener or a raw IP socket to a WebSocket cluster)
and native support for datagram- and byte-streams, steam
multiplexing/demultiplexing, encap/decap, etc.  Considering this
strong emphasis on multiprotocol support, L7mp may actually be closes
...
static CLI tool.  The REST API is likewise similar to [[https://github.com/cncf/udpa][Envoy/xDS]] with
some useful additions, like support for listing, killing, or
dynamically re-routing(!)  individual transport session.

L7mp is written in node.js. This way, it is much simpler and easier to
extend than Envoy or =socat=, but at the same time it is also much
slower. We envision L7mp as an incubator and a test bed for a set of
extensions which should eventually be implemented into those faster
proxies.
...
#+BEGIN_SRC sh
node l7mp.js -c config/l7mp-simple.json -l warn
#+END_SRC

Command line arguments override static configuration parameters. The
sample configuration will fire up a HTTP listener at port 1234 and
...
| STDIO         | N/A               | stream/session   | t     | singleton | no/no/no    | Full     |
...
- STDIO-fork is a (transform-only) protocol for communicating with a
  forked process through STDIO/STDOUT, while STDIO streams to the
  L7mp proxy stdin/stdout (useful for debugging)
