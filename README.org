...
clusters that will be chained sequentially when the listener emits a
session, automatically performing transparent protocol and payload
conversion along the way. Note that datagram boundaries are preserved
during transformation whenever possible, and when it is not (i.e.,
piping a UDP stream to a TCP cluster or L7mp =stdio= will lose
segmentation) L7mp issues a warning.
...
creates a session, and then tries to route the session's stream
through the =up= cluster. This will fail because no one is listening
on the other side.
